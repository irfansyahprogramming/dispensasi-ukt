...
class Functions
{
  ...
  public static function pengajuan($semester)
  {
    if ($semester == ''){
      ...
      {  
        $pengajuan = PengajuanDispensasiUKTModel::where('tb_pengajuan_dispensasi.kode_prodi','like',trim(session('user_unit')).'%')->where('semester',$semester);
      }else{
        $pengajuan = PengajuanDispensasiUKTModel::where('semester',$semester);
      }
