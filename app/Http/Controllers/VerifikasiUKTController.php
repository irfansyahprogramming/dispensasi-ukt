...
class VerifikasiUKTController extends Controller
{
    public function index(Request $request)
    {
        ...
        $pengajuan = DB::table('tb_pengajuan_dispensasi')
            ...
            ->whereIn('tb_pengajuan_dispensasi.status_pengajuan',['1','2','3','4','5','6','7','22'])
            // ->WhereBetween('tb_pengajuan_dispensasi.status_pengajuan',['22','26'])
    ...
    public function verval(Request $request)
    {
        ...
        $pengajuan = DB::table('tb_pengajuan_dispensasi')
            ...
            ->where('kode_prodi', '=', session('user_unit'));
        ...
        return view('verifikasi_dispensasi_prodi', $arrData);
    }
    ...
    public function simpan_verval(Request $request)
    {
        $nim = $request->nim;
        $semester = $request->semester;
        $id = $request->id;
        $kelayakan = $request->pengajuan;
        $alasan = (!$request->txtAlasan)?'':$request->txtAlasan;
        ...
        try {
            ...
            DB::commit();
            return redirect()->route('verifikasi_dispensasi.prodi')->with('toast_success', 'Verifikasi Kelayakan Pengajuan Dispensasi berhasil');
        } catch (Exception $ex) {
            DB::rollBack();
            return redirect()->route('verifikasi_dispensasi.prodi')->with('toast_error', 'Error : ' . $ex->getMessage());
        }
    }
    ...
    public function detil_ajuan($id)
    {
